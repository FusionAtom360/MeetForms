...
class Meet(Base):
    ...
    )  # sign-up deadline, ISO string "YYYY-MM-DD"
    ...
    category_type = Column(
        String, nullable=False, default="age_group"
    )  # "age_group" | "division"
    is_active = Column(
        Boolean, default=False
    )  # only one meet should be active at a time
...
class Entry(Base):
    ...
    division = Column(
        String, nullable=True
    )  # "JV" | "Varsity" | None (for age_group meets)
...
def init_db():
    ...
    if "team_names" not in meet_columns:
        with engine.begin() as connection:
            connection.execute(
                text(
                    "ALTER TABLE meets ADD COLUMN team_names TEXT NOT NULL DEFAULT '[]'"
                )
            )
    ...
    if "category_type" not in meet_columns:
        with engine.begin() as connection:
            connection.execute(
                text(
                    "ALTER TABLE meets ADD COLUMN category_type TEXT NOT NULL DEFAULT 'age_group'"
                )
            )
    ...
    if inspector.has_table("entries"):
        ...
        if "submitted_at" not in entry_columns:
            with engine.begin() as connection:
                connection.execute(
                    text("ALTER TABLE entries ADD COLUMN submitted_at DATETIME")
                )
                connection.execute(
                    text(
                        "UPDATE entries SET submitted_at = CURRENT_TIMESTAMP WHERE submitted_at IS NULL"
                    )
                )
